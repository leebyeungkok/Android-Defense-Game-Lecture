...
public class MainGLRenderer implements Renderer {
    ...
    Soldier [] mSoldier = new Soldier[40];
    Bullet [] mBullet = new Bullet[400];
    int mScore = 0;
    NumberPanel mScorePanel;
    // 터치포인터
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        for(int i=0; i < 40; i++){
            mSoldier[i] = new Soldier(mProgramImage, mProgramSolidColor);
        }
        for(int i=0; i < 400; i++){
            mBullet[i] = new Bullet(mProgramImage, mProgramSolidColor);
        }
        mScorePanel = new NumberPanel(mProgramImage, mProgramSolidColor);
        ...
        mScorePanel.setBitmap( mResourceManager.getHandleNumber());
    ...
    private void renderGame(float[] m){
        ...
        for(int i=0; i < 40; i++){
            mSoldier[i].draw(mMtrxProjectionAndView);
        }
    ...
    public void attackZombie(Soldier soldier){
                ...
                if(Math.abs(mZombie[i].mPosBlockRow - soldier.mPosBlockRow) < 5 &&
                    ...
                    for (int j = 0; j < 400; j++) {
                        if (mBullet[j].getIsActive() == false) {
                            ...
                            mBullet[j].moveToBlock(soldier, mZombie[i], soldier.mPosBlockRow,
                                    soldier.mPosBlockCol, mZombie[i].mPosBlockRow, mZombie[i].mPosBlockCol, this);
                            return;
                        }
                    }
                }
    ...
    private void selectTouchGame(int x, int y){
        if(mButtons[0].isSelected(x, y) == true){
            ...
            for (int j = Map.mInfoSizeCol-1; j >=0;  j--) {
                for (int i = Map.mInfoSizeRow-1; i >=0 ; i--) {
    ...
    int mSoldierIndex = 0;
    // 터치시 아군 추가
    private void addSoldier(int type, int row, int col){
        int tempCount = 0;
        if (type == 0) {
            mSoldier[mSoldierIndex].setBitmap(ResourceManager.getHandleSoldier(), mSoldier[mSoldierIndex].mWidth, mSoldier[mSoldierIndex].mHeight);
        }
        else if (type == 1) {
            mSoldier[mSoldierIndex].setBitmap(ResourceManager.getHandleBigSoldier(), mSoldier[mSoldierIndex].mWidth, mSoldier[mSoldierIndex].mHeight);
        }
